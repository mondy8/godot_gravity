...
func _physics_process(delta):
	...
	# 画面の右側にいる場合、左に移動
	if position.x > Global.SCREEN_WIDTH * 0.6:
		direction = -1
		sprite.set_flip_h(false)
	# 画面の左側にいる場合、右に移動
	elif position.x < Global.SCREEN_WIDTH * 0.4:
		direction = 1
	...
	# 端にいる場合は戻ろうとする
	if position.x < Global.SCREEN_WIDTH * 0.25 or position.x > Global.SCREEN_WIDTH * 0.75:
