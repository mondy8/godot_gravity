...
func fade_in():
	...
	tween.tween_property(self, "modulate", Color(modulate, 0.0), fade_in_duration).finished.connect(
		_on_complete_fade_in
	)
...
func fade_out():
	...
	(
		tween
		. tween_property(self, "modulate", Color(modulate, minimum_opacity), fade_out_duration)
		. finished
		. connect(_on_complete_fade_out)
	)
